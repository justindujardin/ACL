...
Assert::~Assert() { delete mInternal; }

bool Assert::processAssert(Type type, const char *filename, U32 lineNumber,
  ...
  String title = String::ToString("%s: (%s @ %ld)", Internal::smTypeName[type],
                                  filename, lineNumber);
  String fullMessage =
      String::ToString("In function %s:\n\n%s", function, message);
...
bool Assert::isProcessingAssert() const { return mInternal->processing; }
