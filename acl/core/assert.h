...
#define AssertWarn(x, y)                                                       \
  do {                                                                         \
    if (((bool)(x)) == (bool)0)                                                \
      if (Assert::Get().processAssert(Assert::Warning, __FILE__, __LINE__,     \
                                      ACL_PRETTY_FUNCTION, y))                 \
        ACL_DEBUG_BREAK                                                        \
  } while (0)
...
#define AssertFatal(x, y)                                                      \
  do {                                                                         \
    if (((bool)(x)) == (bool)0)                                                \
      if (Assert::Get().processAssert(Assert::Fatal, __FILE__, __LINE__,       \
                                      ACL_PRETTY_FUNCTION, y))                 \
        ACL_DEBUG_BREAK                                                        \
  } while (0)
...
#define AssertISV(x, y)                                                        \
  do {                                                                         \
    if (((bool)(x)) == (bool)0)                                                \
      if (Assert::Get().processAssert(Assert::Fatal_ISV, __FILE__, __LINE__,   \
                                      ACL_PRETTY_FUNCTION, y))                 \
        ACL_DEBUG_BREAK                                                        \
  } while (0)
...
public:
  enum Type {
    ...
    Warning ///< Only triggers in debug build configurations and by default only
            ///< prints to the console
  };

  /// @brief Trigger an assert.
  /// @details This method will block until the assert is somehow dealt with,
  /// and will likely result in the app exiting.
  ...
  /// @param message A message that will be displayed/printed with the assert.
  /// Please make it informational!
  bool processAssert(Type type, const char *filename, U32 lineNumber,
...
#define AssertStatic(expr, msg)                                                \
  do {                                                                         \
    Private::CompileTimeError<((expr) != 0)> ASSERT_##msg;                     \
    (void)ASSERT_##msg;                                                        \
  } while (false)
...
#define AssertStaticNamespace(expr, msg)                                       \
  template <int test> struct ASSERT_##msg {                                    \
    Private::CompileTimeError<test> FAILED;                                    \
  };                                                                           \
...
/*!
   ...
   and use it immediately.  Other functions my use avar too and clobber the
   buffer.
 */
