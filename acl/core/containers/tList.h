...
public:
  /// A PersistentIter is used for the special cases where you need to store an
  /// iterator for efficiency reasons.  The _Iterator class handles the
  /// conversion to/from PersistentIter.
  /// @note The data in the node this points to could go away, so only use these
  /// in cases where
  ///  you control the allocation/deallocation of the nodes in the list.
  typedef PrivatePersist *PersistentIter;

  // Iterator support
  template <typename U, typename E> class _Iterator {
  public:
    typedef U ValueType;
    typedef U *Pointer;
    typedef U &Reference;
    ...
    _Iterator();
    _Iterator(const PersistentIter &iter) { _link = (Link *)iter; }
  ...
  Iterator
  insert(U32 index,
         const Type & = Type()); ///< Insert new element at the given index
  Iterator insert(Iterator,
                  const Type & = Type());    ///< Insert at the given iter
...
template <class Type> List<Type>::~List() { _destroy(); }

//-----------------------------------------------------------------------------

template <class Type>
typename List<Type>::Link *List<Type>::_node(U32 index) const {
...
template <class Type> inline U32 List<Type>::getSize() const { return _size; }
...
template <class Type>
typename List<Type>::Iterator List<Type>::insert(Iterator itr, const Type &x) {
  Link *node = itr._link;
...
template <class Type>
inline typename List<Type>::Iterator List<Type>::insert(U32 index,
                                                        const Type &x) {
  insert(_node(index), x);
}

template <class Type>
inline typename List<Type>::Iterator List<Type>::pushFront(const Type &x) {
  return insert(_head.next, x);
}

template <class Type>
inline typename List<Type>::Iterator List<Type>::pushBack(const Type &x) {
...
template <class Type> inline void List<Type>::popFront() { erase(_head.next); }

template <class Type> inline void List<Type>::popBack() { erase(_head.prev); }
...
template <class Type>
inline typename List<Type>::Iterator List<Type>::rbegin() {
  return _head.prev;
}

template <class Type>
inline typename List<Type>::ConstIterator List<Type>::rbegin() const {
  return _head.prev;
}
...
template <class Type>
inline typename List<Type>::ConstIterator List<Type>::rend() const {
  return &_head;
}
...
template <class Type>
inline const Type &List<Type>::operator[](U32 index) const {
  return static_cast<Node *>(_node(index))->value;
}
...
inline U *List<Type>::_Iterator<U, E>::operator->() const {
  ...
  return &((typename Select<IsConst<E>::True, const Node *, Node *>::ValueType)
               _link)
              ->value;
}
