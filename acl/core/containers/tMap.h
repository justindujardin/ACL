...
template <class A, class B> struct CompoundKey {
  A key1;
  B key2;
  ...
  bool operator==(const CompoundKey &compound) const {
    return key1 == compound.key1 && key2 == compound.key2;
  }
};

template <class A, class B, class C> struct CompoundKey3 {
  A key1;
  B key2;
  C key3;
  ...
  bool operator==(const CompoundKey3 &compound) const {
    return key1 == compound.key1 && key2 == compound.key2 &&
           key3 == compound.key3;
  }
};
...
namespace DictHash {
inline U32 hash(U32 data) { return data; }

inline U32 hash(const StringCase &data) { return data.getHashCaseSensitive(); }

inline U32 hash(const StringNoCase &data) {
  return data.getHashCaseInsensitive();
}

inline U32 hash(const String &data) { return data.getHashCaseInsensitive(); }

inline U32 hash(const char *data) {
  return hashPointer((const U8 *)data, dStrlen(data), 0);
}

inline U32 hash(const void *data) { return (U32)data; }

template <class A, class B> inline U32 hash(const CompoundKey<A, B> &compound) {
...
namespace KeyCmp {
template <typename Key> inline bool equals(const Key &keya, const Key &keyb) {
  return (keya == keyb);
}
...
template <> inline bool equals<>(const String &keya, const String &keyb) {
  return (keya.equal(keyb, String::NoCase));
}
...
private:
  struct Node {
    ...
    Pair mPair;
    Node() : mNext(0) {}
    Node(Pair p, Node *n) : mNext(n), mPair(p) {}
  };
...
public:
  // Iterator support
  template <typename U, typename E, typename M> class _Iterator {
  ...
  public:
    ...
    bool operator!=(const _Iterator &b) const { return !(*this == b); }

    U *operator->() const { return &mLink->mPair; }
  ...
  void erase(const Key &key,
             const Value &value); ///< Erase entry for this key-value pair

  // HashTable lookup
  Iterator findOrInsert(const Key &key);
  Iterator find(const Key &); ///< Find the first entry for the given key
  ConstIterator
  find(const Key &) const; ///< Find the first entry for the given key
  bool find(const Key &key,
            Value &value); ///< Find the first entry for the given key
  S32 count(
      const Key &) const; ///< Count the number of matching keys in the table
...
HashTable<Key, Value>::HashTable() : mNodeAllocator(512) {
  mTableSize = 0;
  mTable = 0;
  mSize = 0;
}
...
template <typename Key, typename Value>
HashTable<Key, Value>::HashTable(const HashTable &p) : mNodeAllocator(512) {
  mSize = 0;
  mTableSize = 0;
  mTable = 0;
  *this = p;
...
template <typename Key, typename Value>
typename HashTable<Key, Value>::Node *
HashTable<Key, Value>::_next(U32 index) const {
...
template <typename Key, typename Value> void HashTable<Key, Value>::_destroy() {
...
/// This insert method does not insert duplicate keys. If the key already exists
/// in the table the function will fail and end() is returned.
template <typename Key, typename Value>
typename HashTable<Key, Value>::Iterator
HashTable<Key, Value>::insertUnique(const Key &key, const Value &x) {
...
template <typename Key, typename Value>
typename HashTable<Key, Value>::Iterator
HashTable<Key, Value>::insertEqual(const Key &key, const Value &x) {
  if (mSize >= mTableSize)
    _resize(mSize + 1);
...
template <typename Key, typename Value>
typename HashTable<Key, Value>::Iterator
HashTable<Key, Value>::findOrInsert(const Key &key) {
  if ((S32)mSize >= mTableSize)
    _resize(mSize + 1);
  Node **table = &mTable[_index(key)];
  for (Node *itr = *table; itr; itr = itr->mNext)
    if (KeyCmp::equals<Key>(itr->mPair.key, key))
...
template <typename Key, typename Value>
typename HashTable<Key, Value>::Iterator
HashTable<Key, Value>::find(const Key &key) {
  if (mTableSize)
...
template <typename Key, typename Value>
typename HashTable<Key, Value>::ConstIterator
HashTable<Key, Value>::find(const Key &key) const {
  if (mTableSize) {
    for (Node *itr = mTable[_index(key)]; itr; itr = itr->mNext) {
      if (KeyCmp::equals<Key>(itr->mPair.key, key))
...
template <typename Key, typename Value>
inline typename HashTable<Key, Value>::ConstIterator
HashTable<Key, Value>::begin() const {
...
public:
  ...
  bool contains(const Key &a) const { return mMap.count(a) > 0; }
  /// Try to get the value of the specified key.  Returns true and fills in the
  /// value if the key exists.  Returns false otherwise. Unlike operator [],
  /// this function does not create the key/value if the key is not found.
  ...
  // operators
  /// Index using the given key. If the key is not currently in the map it is
  /// added. If you just want to try to get the value without the side effect of
  /// creating the key, use tryGetValue() instead.
...
Map<Key, Value, Sequence>::Map(const Map &p) {
  *this = p;
}
...
template <typename Key, typename Value, class Sequence>
typename Map<Key, Value, Sequence>::Iterator
Map<Key, Value, Sequence>::insert(const Key &key, const Value &x) {
...
template <typename Key, typename Value, class Sequence>
typename Map<Key, Value, Sequence>::Iterator
Map<Key, Value, Sequence>::find(const Key &key) {
  return mMap.find(key);
}

template <typename Key, typename Value, class Sequence>
typename Map<Key, Value, Sequence>::ConstIterator
Map<Key, Value, Sequence>::find(const Key &key) const {
  return mMap.find(key);
}
...
template <typename Key, typename Value, class Sequence>
inline typename Map<Key, Value, Sequence>::Iterator
Map<Key, Value, Sequence>::begin() {
  return mMap.begin();
}

template <typename Key, typename Value, class Sequence>
inline typename Map<Key, Value, Sequence>::ConstIterator
Map<Key, Value, Sequence>::begin() const {
  return mMap.begin();
}

template <typename Key, typename Value, class Sequence>
inline typename Map<Key, Value, Sequence>::Iterator
Map<Key, Value, Sequence>::end() {
  return mMap.end();
}

template <typename Key, typename Value, class Sequence>
inline typename Map<Key, Value, Sequence>::ConstIterator
Map<Key, Value, Sequence>::end() const {
  return mMap.end();
}
