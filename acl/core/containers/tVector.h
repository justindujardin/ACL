...
// If you call reserve, or setSize then the vector allocates enough blocks of VectorBlockSize to fulfill the request.
// So if the size of the elements being stored in the vector is 1000 bytes, VectorBlockSize is 16, and you call setSize(23), it'll allocate 32000 bytes to 
// be able to store 32 elements (2 * 16).
// 
// If the array automatically grows by calls to push_back, insert, or increment then the vector will double it's capacity to meet the request.  Another example:
// vector.size = 255, capacity = 255 and a push back occurs, then vector.size = 256 and capcity = 512
//
// The reasoning behind this is that if you know your memory needs up front, then we want vector's allocation to match that as close as possible.  But if you're
// accumlating things and don't have a good handle on what your memory needs will be, then let's use an optimal (performance wise) allocation strategy at the
// possible expense of waste.
...
namespace ACLib
{
   ...
   inline U32 VectorBase::capacity() const
   {
      return mElementsAllocated;
   }
   ...
   inline void* VectorBase::address() const
   {
      return mArray;
   }
   ...
   inline void VectorBase::erase_fast(U32 index)
   {
      ...
      //   size of the vector.
      if (index < (mElementCount - 1))
         dMemmove(&mArray[index * mElementSize], &mArray[(mElementCount - 1) * mElementSize], mElementSize);
      mElementCount--;
   }

   inline void VectorBase::clear()
   {
      mElementCount = 0;
   }

   inline void VectorBase::compact()
   {
      resize(mElementCount);
   }
   ...
   template<class T> inline Vector<T>::Vector(const U32 initialSize) : VectorBase(sizeof(T), initialSize)
   {
   }
