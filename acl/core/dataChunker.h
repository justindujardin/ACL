...
public:
   ...
   /// Allows one to change the size of the chunks allocated.  Returns true if successful.  DataChunker must
   /// be cleared to a clean slate (via new construction, or call to ::freeBlocks) for this method to succeed.
...
public:
   ...
   /// Allocates space for a new class but relies on the caller to construct in place
   /// This allows the caller to do a constructor other than default.  The caller
   /// *MUST* construct the pointer return as it'll be destructed in free and freeBlocks.
   T *allocRaw()
   {
   ...
   // This is more complicated than DataChunker::freeBlocks because we need to call
   // the destructor on our items.
   void freeBlocks()
   {
      // Make sure the freelist blocks are valid for destruction.  This seems dumb,
      // but I can't think of another bulletproof way to handle the already freed items.
      // You can't mark it NULL because that may be a valid value for the first 4 bytes 
      // of a class.  
