...
///   F32 * ptr =
///   (F32*)FrameAllocator::alloc(sizeof(F32)*2*targetMesh->vertsPerFrame);
///
///   ... calculations ...
///
...
void *FrameAllocator::alloc(const U32 allocSize) {
  ...
  AssertFatal(smWaterMark + _allocSize <= smHighWaterMark,
  ...
  smWaterMark =
      (smWaterMark + (ACL_BYTE_ALIGNMENT - 1)) & (~(ACL_BYTE_ALIGNMENT - 1));

  // Sanity check.
  AssertFatal(!(smWaterMark & (ACL_BYTE_ALIGNMENT - 1)),
              "Frame allocation is not on a 4-byte boundry.");
...
U32 FrameAllocator::getWaterMark() { return smWaterMark; }

U32 FrameAllocator::getHighWaterMark() { return smHighWaterMark; }
...
/// don't have to remember to reset the FrameAllocator on every posssible
/// branch.
class FrameAllocatorMarker {
  U32 mMarker;

public:
  FrameAllocatorMarker() { mMarker = FrameAllocator::getWaterMark(); }

  ~FrameAllocatorMarker() { FrameAllocator::setWaterMark(mMarker); }
...
/// @code
/// FrameTemp<char> tempStr(32); // NOTE! This parameter is NOT THE SIZE IN
/// BYTES. See constructor docs. dStrcat( tempStr, SomeOtherString ); tempStr[2]
/// = 'l'; Con::printf( tempStr ); Con::printf( "Foo: %s", ~tempStr );
/// @endcode
///
...
template <class T> class FrameTemp {
protected:
  U32 mWaterMark;
  T *mMemory;
...
public:
  /// Constructor will store the FrameAllocator watermark and allocate the
  /// memory off of the FrameAllocator.
  ///
  /// @note It is important to note that, unlike the FrameAllocatorMarker and
  /// the FrameAllocator itself, the argument to allocate is NOT the size in
  /// bytes, doing:
  /// @code
  ...
  FrameTemp(const U32 count = 1) : mNumObjectsInMemory(count) {
    AssertFatal(count > 0,
                "Allocating a FrameTemp with less than one instance");
  ...
  /// NOTE: This will dereference the memory, NOT do standard unary plus
  /// behavior
  T &operator+() { return *mMemory; };
  /// NOTE: This will dereference the memory, NOT do standard unary plus
  /// behavior
...
#define FRAME_TEMP_NC_SPEC(type)                                               \
  template <> inline FrameTemp<type>::FrameTemp(const U32 count) {             \
    AssertFatal(count > 0,                                                     \
                "Allocating a FrameTemp with less than one instance");         \
    mWaterMark = FrameAllocator::getWaterMark();                               \
    mMemory =                                                                  \
        reinterpret_cast<type *>(FrameAllocator::alloc(sizeof(type) * count)); \
  }                                                                            \
  template <> inline FrameTemp<type>::~FrameTemp() {                           \
    FrameAllocator::setWaterMark(mWaterMark);                                  \
  }
