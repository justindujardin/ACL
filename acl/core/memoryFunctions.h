...
// Mac OS X always aligns malloc'd memory on 16-byte boundaries, so just use
// normal malloc routines
...
namespace ACLib {
inline void *dMalloc(dsize_t in_size) { return _MALLOC(in_size); }

inline void *dRealloc(void *in_pResize, dsize_t in_size) {
  return _REALLOC(in_pResize, in_size);
}

inline void dFree(void *in_pFree) { _FREE(in_pFree); }
