...
FileStream *FileStream::createAndOpen(const String &inFileName,
                                      FS::File::AccessMode inMode) {
...
U32 FileStream::getPosition() const {
  AssertFatal(0 != mStreamCaps,
              "FileStream::getPosition: the stream isn't open");
  // AssertFatal(true == hasCapability(StreamPosition),
  // "FileStream::getPosition(): lacks positioning capability");

  // return the position inside the buffer if its valid, otherwise return the
...
bool FileStream::setPosition(const U32 i_newPosition) {
  AssertFatal(0 != mStreamCaps,
              "FileStream::setPosition: the stream isn't open");
  AssertFatal(hasCapability(StreamPosition),
              "FileStream::setPosition: lacks positioning capability");

  // if the buffer is valid, test the new position against the bounds of the
  // buffer
  if ((BUFFER_INVALID != mBuffHead) && (i_newPosition >= mBuffHead) &&
      (i_newPosition <= mBuffTail)) {
    // set the position and return
    mBuffPos = i_newPosition;

    // FIXME [tom, 9/5/2006] This needs to be checked. Basically, when seeking
    // within the buffer, if the stream has an EOS status before the seek then
    // if you try to read immediately after seeking, you'll incorrectly get an
    // EOS.
    //
    // I am not 100% sure if this fix is correct, but it seems to be working for
    // the undo system.
  ...
  // otherwise the new position lies in some block not in memory
    ...
    mFile->setPosition(i_newPosition, FS::File::Begin);

    setStatus();

    if (mFile->getStatus() == FS::FileNode::EndOfFile)
      mEOF = true;
...
U32 FileStream::getStreamSize() {
  AssertWarn(0 != mStreamCaps,
             "FileStream::getStreamSize: the stream isn't open");
  AssertFatal((BUFFER_INVALID != mBuffHead && true == mDirty) ||
                  false == mDirty,
              "FileStream::getStreamSize: buffer must be valid if its dirty");

  // the stream size may not match the size on-disk if its been written to...
  if (mDirty)
    return (getMax((U32)(mFile->getSize()),
                   mBuffTail + 1)); ///<@todo U64 vs U32 issue
...
//-----------------------------------------------------------------------------
bool FileStream::open(const String &inFileName, FS::File::AccessMode inMode) {
  AssertWarn(0 == mStreamCaps,
             "FileStream::setPosition: the stream is already open");
  ...
  if (inMode == FS::File::Write || inMode == FS::File::WriteAppend)
    FS::CreatePath(filePath);

  mFile = FS::OpenFile(filePath, inMode);

  if (mFile != NULL) {
    setStatus();
    switch (inMode) {
    case FS::File::Read:
      mStreamCaps = U32(StreamRead) | U32(StreamPosition);
      break;
    case FS::File::Write:
    case FS::File::WriteAppend:
      mStreamCaps = U32(StreamWrite) | U32(StreamPosition);
      break;
    case FS::File::ReadWrite:
      mStreamCaps = U32(StreamRead) | U32(StreamWrite) | U32(StreamPosition);
      break;
    default:
      AssertFatal(false,
                  String::ToString("FileStream::open: bad access mode on %s",
                                   inFileName.c_str()));
    }
  } else {
...
void FileStream::close() {
  ...
  if (mFile != NULL) {
    ...
    AssertFatal(mFile->getStatus() == FS::FileNode::Closed,
                "FileStream::close: close failed");
...
bool FileStream::flush() {
  ...
  AssertFatal(false == mDirty || BUFFER_INVALID != mBuffHead,
              "FileStream::flush: buffer must be valid if its dirty");

  // if the buffer is dirty
  if (mDirty) {
    AssertFatal(hasCapability(StreamWrite),
                "FileStream::flush: a buffer without write-capability should "
                "never be dirty");

    // align the file pointer to the buffer head
    if (mBuffHead != mFile->getPosition()) {
      mFile->setPosition(mBuffHead, FS::File::Begin);
      if (mFile->getStatus() != FS::FileNode::Open &&
          mFile->getStatus() != FS::FileNode::EndOfFile)
        return (false);
    }
    ...
    mFile->write((char *)mBuffer + (mBuffHead - blockHead),
                 mBuffTail - mBuffHead + 1);
...
bool FileStream::_write(const U32 i_numBytes, const void *i_pBuffer) {
  ...
  AssertFatal(NULL != i_pBuffer || i_numBytes == 0,
              "FileStream::_write: NULL source buffer pointer on non-zero "
              "write request");
  ...
  if (0 != i_numBytes) {
    ...
    if (BUFFER_INVALID != mBuffHead) {
      ...
      AssertFatal(0 == writeSize || (mBuffPos - blockHead) < BUFFER_SIZE,
                  "FileStream::_write: out of bounds buffer position");
...
bool FileStream::fillBuffer(const U32 i_startPosition) {
  AssertFatal(0 != mStreamCaps,
              "FileStream::fillBuffer: the stream isn't open");
  AssertFatal(false == mDirty,
              "FileStream::fillBuffer: buffer must be clean to fill");

  // make sure start position and file pointer jive
  if (i_startPosition != mFile->getPosition()) {
    mFile->setPosition(i_startPosition, FS::File::Begin);
    if (mFile->getStatus() != FS::FileNode::Open &&
        mFile->getStatus() != FS::FileNode::EndOfFile) {
      setStatus();
      return (false);
  ...
  // otherwise
    ...
    U32 bytesRead = mFile->read((char *)mBuffer + (i_startPosition - blockHead),
                                BUFFER_SIZE - (i_startPosition - blockHead));
    setStatus();
    if (Ok == getStatus() || EOS == getStatus()) {
...
void FileStream::calcBlockHead(const U32 i_position, U32 *o_blockHead) {
  AssertFatal(NULL != o_blockHead,
              "FileStream::calcBlockHead: NULL pointer passed for block head");

  *o_blockHead = i_position / BUFFER_SIZE * BUFFER_SIZE;
}

//-----------------------------------------------------------------------------
void FileStream::calcBlockBounds(const U32 i_position, U32 *o_blockHead,
                                 U32 *o_blockTail) {
  AssertFatal(
      NULL != o_blockHead,
      "FileStream::calcBlockBounds: NULL pointer passed for block head");
  AssertFatal(
      NULL != o_blockTail,
      "FileStream::calcBlockBounds: NULL pointer passed for block tail");
...
void FileStream::setStatus() {
  ...
  case FS::FileNode::Open:
    Stream::setStatus(Ok);
    break;

  case FS::FileNode::Closed:
    Stream::setStatus(Closed);
    break;

  case FS::FileNode::EndOfFile:
    Stream::setStatus(EOS);
    break;

  case FS::FileNode::FileSystemFull:
  case FS::FileNode::NoSuchFile:
  case FS::FileNode::NoDisk:
  case FS::FileNode::SharingViolation:
    Stream::setStatus(IOError);
    break;

  case FS::FileNode::IllegalCall:
    Stream::setStatus(IllegalCall);
    break;

  case FS::FileNode::UnknownError:
