...
namespace ACLib
{
   ...
   FileStream *FileStream::createAndOpen(const String &inFileName, FS::File::AccessMode inMode)
   {
   ...
   void FileStream::close()
   {
      ...
      if (mFile != NULL )
      {
         ...
         AssertFatal(mFile->getStatus() == FS::FileNode::Closed, "FileStream::close: close failed");
   ...
   bool FileStream::flush()
   {
      ...
      if (mDirty)
      {
         ...
         if (mBuffHead != mFile->getPosition())
         {
            ...
            if (mFile->getStatus() != FS::FileNode::Open && mFile->getStatus() != FS::FileNode::EndOfFile)
               return(false);
         }
