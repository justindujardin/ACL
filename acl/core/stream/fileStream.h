...
public:
  enum {
    BUFFER_SIZE =
        8 * 1024, // this can be changed to anything appropriate [in k]
...
public:
  ...
  static FileStream *createAndOpen(const String &inFileName,
  ...
  virtual bool open(const String &inFileName, FS::File::AccessMode inMode);
  virtual void close();
...
protected:
  ...
  static void calcBlockBounds(const U32 i_position, U32 *o_blockHead,
                              U32 *o_blockTail);
  void setStatus();
...
private:
  FS::FileRef mFile; // file being streamed
  ...
  FileStream &
  operator=(const FileStream &i_fileStrm); // disable assignment operator
};
};     // namespace ACLib
