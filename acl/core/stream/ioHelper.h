...
/// template expansion.
namespace IOHelper {
template <class A> inline bool reads(Stream *s, A &a) {
  U32 expected;
  s->read(&expected);
  AssertFatal(expected == sizeof(A), "Incorrect object size");
  return s->read(&a);
}
...
template <class A, class B> inline bool reads(Stream *s, A &a, B &b) {
  reads(s, a);
  return reads(s, b);
}

template <class A> inline bool writes(Stream *s, A &a) {
  s->write(U32(sizeof(A)));
  return s->write(a);
}
...
template <class A, class B> inline bool writes(Stream *s, A &a, B &b) {
  writes(s, a);
  return writes(s, b);
}
} // namespace IOHelper
