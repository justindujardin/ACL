...
#define IMPLEMENT_OVERLOADED_READ(type)                                        \
  bool Stream::read(type *out_read) { return read(sizeof(type), out_read); }

#define IMPLEMENT_OVERLOADED_WRITE(type)                                       \
  bool Stream::write(type in_write) { return write(sizeof(type), &in_write); }
...
#if defined(ACL_OS_MAC)
      U32 pushPos = getPosition(); // in case we need to back up.
      if (read(buff)) // feeling free to overwrite the \r as the NULL below will
                      // overwrite again...
...
bool Stream::write(const ByteBuffer &rd) {
  bool s = write(rd.getBufferSize());
  s &= write(rd.getBufferSize(), rd.getBuffer());
  return s;
}

bool Stream::read(ByteBuffer *rd) {
  U32 size = 0;
  bool s = read(&size);
