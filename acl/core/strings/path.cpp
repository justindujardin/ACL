...
String Path::CleanSeparators(String path) { return path.replace('\\', '/'); }
...
String Path::MakeRelativePath(String makeRelative, String relativeTo) {
  ...
  S32 maxIterations =
      getMin(makeRelPath.getDirectoryCount(), relToPath.getDirectoryCount());
  ...
  for (S32 i = 0; i < (S32)makeRelPath.getDirectoryCount(); i++) {
    // Same?
    if (!makeRelPath.getDirectory(i).equal(relToPath.getDirectory(i),
                                           String::NoCase)) {
      // No, do we need a ../?
      if (i <= maxIterations)
        outPathRel += "../";
...
bool Path::isDirectory() const { return mFile.isEmpty() && mExt.isEmpty(); }
