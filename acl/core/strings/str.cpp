...
namespace DictHash {
U32 hash(String::StringData *);
}
namespace KeyCmp {
template <typename Key> bool equals(const Key &, const Key &);
template <>
bool equals<>(String::StringData *const &, String::StringData *const &);
} // namespace KeyCmp
...
struct DeleteString {
  template <class T> static void destroy(T *ptr) { dFree(ptr); }
};
...
/// If pos is non-zero, then in mode StrLeft the search starts at (hay + pos)
/// and in mode StrRight the search starts at (hay + pos - 1)
/// @return Returns a pointer to the location of the character in the haystack
/// or 0
...
static const char *StrFind(const char *hay, const char *needle, S32 pos,
                           U32 mode) {
  if (mode & String::Right) {
    const char *he = hay;
...
protected:
  ...
  U32 mRefCount; ///< String reference count; string is not refcounted if this
                 ///< is U32_MAX (necessary for thread-safety of interned
                 ///< strings and the empty string).
  U32 mLength;   ///< String length in bytes excluding null.
  mutable U32 mNumChars;   ///< Character count; varies from byte count for
                           ///< strings with multi-bytes characters.
  ...
  StringData()
      : mRefCount(U32_MAX), mLength(0), mNumChars(0), mHashCase(U32_MAX),
        mHashNoCase(U32_MAX), mUTF16(new UTF16[1]), mData(),
        mIsInterned(false) {
...
public:
  ...
  StringData(const StringChar *data, bool interned = false)
      : mRefCount(1), mNumChars(U32_MAX), mHashCase(U32_MAX),
        mHashNoCase(U32_MAX), mUTF16(NULL), mIsInterned(interned) {
  ...
  U32 getDataSize() const { return (mLength + 1); }
  U32 getDataSizeUTF16() const { return (mLength * sizeof(UTF16)); }
  UTF8 operator[](U32 index) const {
    AssertFatal(index < mLength,
                "String::StringData::operator []() - index out of range");
    return mData[index];
  }
  UTF8 *utf8() { return mData; }
  const UTF8 *utf8() const { return mData; }
  UTF16 *utf16() const {
  ...
  U32 getHashCase() const { return mHashCase; }
  U32 getOrCreateHashCase() const {
    if (mHashCase == U32_MAX)
      mHashCase = hashPointer((const U8 *)(mData),
                              getMin(mLength, (U32)MAX_HASH_LENGTH), 0);
    return mHashCase;
  }
  U32 getHashNoCase() const { return mHashNoCase; }
  U32 getOrCreateHashNoCase() const {
    if (mHashNoCase == U32_MAX) {
      ...
      mHashNoCase = hashPointer((const U8 *)(sLowerBuffer), len, 0);
    }
  ...
  bool isInterned() const { return mIsInterned; }
...
namespace KeyCmp {
template <>
inline bool equals<>(String::StringData *const &d1,
                     String::StringData *const &d2) {
...
/// is because we really shouldn't depend on dtor ordering within this file and
/// thus we can't tell whether the intern string memory is freed before or after
/// the table is destroyed.
typedef HashTable<String::StringData *, String::StringData *> StringTable;

#ifdef STR_THREADSAFE_INTERN
static Platform::Mutex sStringTableLock;
#endif
...
void *String::StringData::operator new(size_t size, U32 len) {
  AssertFatal(len != 0,
              "String::StringData::operator new() - string must not be empty");
  StringData *str =
      reinterpret_cast<StringData *>(dMalloc(size + len * sizeof(StringChar)));
...
void String::StringData::operator delete(void *ptr) {
  ...
  dFetchAndAdd(
      sgStringMemBytes,
      U32(-(S32(sizeof(StringData) + sub->mLength * sizeof(StringChar)))));
...
void *String::StringData::operator new(size_t size, U32 len,
  ...
  AssertFatal(len != 0,
              "String::StringData::operator new() - string must not be empty");
  StringData *str = reinterpret_cast<StringData *>(
      chunker.alloc(size + len * sizeof(StringChar)));
...
String::String() { _string = StringData::Empty(); }
...
String String::intern() const {
  ...
  Platform::Mutex::ScopedLock scopeLock(sStringTableLock);
#endif
  ...
  // Create new.
  StringData *data =
      new (length(), sStringTableChunker) StringData(c_str(), true);
...
const StringChar *String::c_str() const { return _string->utf8(); }

const UTF16 *String::utf16() const { return _string->utf16(); }

String::SizeType String::length() const { return _string->getLength(); }

String::SizeType String::size() const { return _string->getDataSize(); }

String::SizeType String::numChars() const { return _string->getNumChars(); }

bool String::isEmpty() const { return (_string == StringData::Empty()); }

bool String::isShared() const { return _string->isShared(); }

bool String::isSame(const String &str) const {
  return (_string == str._string);
}

bool String::isInterned() const { return (_string->isInterned()); }
...
bool String::operator==(const String &str) const {
  if (str._string == _string)
    ...
    return (dMemcmp(str._string->utf8(), _string->utf8(),
                    _string->getLength()) == 0);
}
...
String::SizeType String::find(const StringChar *str, SizeType pos,
                              U32 mode) const {
  AssertFatal(str, "String:: Invalid null ptr argument");
...
String &String::insert(SizeType pos, const StringChar *str, SizeType len) {
  ...
  AssertFatal((pos <= lena),
              "Calling String::insert with position greater than length");
  U32 newlen = lena + len;
  ...
  if (!newlen)
    ...
    String::copy(sub->utf8() + pos + len, _string->utf8() + pos,
                 lena - pos + 1);
  }
...
String &String::erase(SizeType pos, SizeType len) {
  AssertFatal(len != 0,
              "String::erase() - Calling String::erase with 0 length");
  AssertFatal((pos + len) <= length(),
              "String::erase() - Invalid string region");
  ...
  if (!newlen)
    ...
    String::copy(sub->utf8() + pos, _string->utf8() + pos + len,
                 slen - (pos + len) + 1);
  }
...
String &String::replace(SizeType pos, SizeType len, const StringChar *str) {
  ...
  AssertFatal((pos + len) <= length(),
              "String::replace() - Invalid string region");
  ...
  if (!newlen)
    ...
    String::copy(sub->utf8() + pos + rlen, _string->utf8() + pos + len,
                 slen - pos - len + 1);
  }
...
String &String::replace(const String &s1, const String &s2) {
  ...
  if (newSize == 1)
    ...
    for (U32 i = 0; i < (U32)indices.size(); i++) {
      ...
      if (copyCharCount > 0) {
        dMemcpy(sub->utf8() + newPos, _string->utf8() + pos,
                copyCharCount * sizeof(StringChar));
        newPos += copyCharCount;
      }

      // Copy over the replacement string.
      if (s2len > 0)
        dMemcpy(sub->utf8() + newPos, s2._string->utf8(),
                s2len * sizeof(StringChar));
...
S32 String::StrFormat::formatAppend(const char *format, void *args) {
  ...
  if (_dynamicBuffer == NULL) {
    _len += vsnprintf(_fixedBuffer + _len, sizeof(_fixedBuffer) - _len, format,
                      *(va_list *)args);
  ...
  _len += vsnprintf(_dynamicBuffer + _len, _dynamicSize - _len, format,
                    *(va_list *)args);
...
String String::SpanToString(const char *start, const char *end) {
  ...
  AssertFatal(
      ...
      "Invalid arguments to String::SpanToString - end is before start");
