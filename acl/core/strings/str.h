...
public:
  ...
  static const SizeType
      NPos; ///< Indicates 'not found' when using find() functions
  ...
  SizeType size() const;   ///< Returns the length of the string in bytes
                           ///< including the NULL terminator.
  SizeType
  numChars() const;     ///< Returns the length of the string in characters.
  ...
  bool isSame(const String &str)
      const; ///< Return true if both strings refer to the same shared data.

  U32 getHashCaseSensitive()
      const; ///< Get the case-sensitive hash of the string [only calculates the
             ///< hash as necessary]
  U32 getHashCaseInsensitive()
      const; ///< Get the case-insensitive hash of the string  [only calculates
             ///< the hash as necessary]
  ...
  */
  S32 compare(const StringChar *str, SizeType len = 0,
              U32 mode = Case | Left) const;
  S32 compare(const String &str, SizeType len = 0, U32 mode = Case | Left)
      const; ///< @see compare(const StringChar *, SizeType, U32) const
  ...
  */
  ...
  SizeType find(const StringChar *str, SizeType pos = 0,
                U32 mode = Case | Left) const;
  SizeType find(const String &str, SizeType pos = 0,
                U32 mode = Case | Left) const;
...
public:
  ...
  */
  class StrFormat {
  public:
    StrFormat() : _dynamicBuffer(NULL), _dynamicSize(0), _len(0) {
      _fixedBuffer[0] = '\0';
    }

    StrFormat(const char *formatStr, void *args)
        : _dynamicBuffer(NULL), _dynamicSize(0), _len(0) {
    ...
    const char *c_str() {
      return _dynamicBuffer ? _dynamicBuffer : _fixedBuffer;
    }
    ...
    /// Copy the formatted string into the output buffer which must be at least
    /// size() characters.
    char *copy(char *buffer) const;

    /// Return the length of the formated string (does not include the
    /// terminating 0)
...
private:
  String(StringData *str) : _string(str) {}
...
public:
  ...
  U32 length() const { return mFormat.length(); }

  void copy(char *buffer) const { mFormat.copy(buffer); }

  String end() { return mFormat.getString(); }
  ...
  S32 append(const char *str) { return mFormat.append(str); }
...
// For use in hash tables and the like for explicitly requesting case sensitive
// hashing. Meant to only appear in hash table definition (casting will take
// care of the rest).
...
// For use in hash tables and the like for explicitly requesting case
// insensitive hashing. Meant to only appear in hash table definition (casting
