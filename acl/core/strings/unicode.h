...
/// - <b>Code Point</b>: a single character of Unicode text. Used to
/// disabmiguate from C char type.
/// - <b>UTF-32</b>: a Unicode encoding format where one code point is always 32
/// bits wide.
///   This format can in theory contain any Unicode code point that will ever be
///   needed, now or in the future. 4billion+ code points should be enough,
///   right?
/// - <b>UTF-16</b>: a variable length Unicode encoding format where one code
/// point can be
///   either one or two 16-bit code units long.
/// - <b>UTF-8</b>: a variable length Unicode endocing format where one code
/// point can be
///   up to four 8-bit code units long. The first bit of a single byte UTF-8
///   code point is 0. The first few bits of a multi-byte code point determine
///   the length of the code point.
///   @see http://en.wikipedia.org/wiki/UTF-8
/// - <b>Surrogate Pair</b>: a pair of special UTF-16 code units, that encode a
/// code point
///   that is too large to fit into 16 bits. The surrogate values sit in a
///   special reserved range of Unicode.
/// - <b>Code Unit</b>: a single unit of a variable length Unicode encoded code
/// point.
///   UTF-8 has 8 bit wide code units. UTF-16 has 16 bit wide code units.
/// - <b>BMP</b>: "Basic Multilingual Plane". Unicode values U+0000 - U+FFFF.
/// This range
///   of Unicode contains all the characters for all the languages of the world,
///   that one would usually be interested in. All code points in the BMP are 16
///   bits wide or less.

/// The current implementation of these conversion functions deals only with the
/// BMP. Any code points above 0xFFFF, the top of the BMP, are replaced with the
...
///  This means that UTF-8 output is clamped to 3 code units ( bytes ) per code
///  point.
...
//-----------------------------------------------------------------------------
/// Functions that convert buffers of unicode code points, into a provided
/// buffer.
...
/// - Conversion to UTF-8 requires a buffer of 3 bytes (U8's) per character,
/// + 1.
/// - Conversion to UTF-16 requires a buffer of 1 U16 (2 bytes) per character,
/// + 1.
/// - Conversion to UTF-32 requires a buffer of 1 U32 (4 bytes) per character,
/// + 1.
...
///   allocate buffers.
/// - oneUTF8toUTF32() and oneUTF16toUTF32() return the converted Unicode code
/// point
///   in *codepoint, and set *unitsWalked to the \# of code units *codepoint
///   took up. The next Unicode code point should start at *(codepoint +
///   *unitsWalked).
/// - oneUTF32toUTF8()  requires a 3 byte buffer, and returns the \# of bytes
/// used.
...
/// - If *unistring does not point to a null terminated string of the correct
/// type,
