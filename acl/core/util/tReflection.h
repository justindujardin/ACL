...
#define TypeInfoTemplate(name, result)                                         \
  template <typename T> struct name {                                          \
    static const bool True = result, False = !True;                            \
  }
...
#define TypeInfoPartialSpec1(name, type, spec, result)                         \
  template <type> struct name<spec> {                                          \
    static const bool True = result, False = !True;                            \
  }
#define TypeInfoPartialSpec2(name, type1, type2, spec, result)                 \
  template <type1, type2> struct name<spec> {                                  \
    static const bool True = result, False = !True;                            \
  }
...
template <typename T> struct NumericLimits {
  ...
  static int epsilon() {
    return 0;
  } ///< Difference between 1 and the least value greater than 1 possible
};
...
TypeLimitTemplate(F64, 2.2250738585072014e-308, 1.7976931348623157e+308, false,
...
///   <dl>
///      <dt>IsPod<dd>POD classes are "Plain Old Data". They have trivial
///      constructors,
///         destructors and assignment operators as well as no complex data
///         member.
...
///         nothing and can be skipped.
///      <dt>HasTrivialAssign<dd>This class can be assigned using memcpy instead
///      of
///         invoking the assignment operator.
///      <dt>HasTrivialCopy<dd>This class doesn't care where it's located in
///      memory
...
// IsConvertible<T,U>
namespace Private {
template <typename T> struct IsConvertible_aux {
  struct anything {
...
template <typename T> struct IsArithmetic {
  static const bool True = IsIntegral<T>::True || IsFloat<T>::True;
  static const bool False = !True;
};
...
template <typename T> struct IsAbstract : public IsFunction<T> {};
...
template <typename T> struct IsPointer {
  static const bool True =
      Private::IsPointer_aux<T>::True && IsMemberPointer<T>::False;
  static const bool False = !True;
};
...
template <typename T> struct IsEnum {
  ...
  static const bool True = Private::IsEnumConvertible < T,
                    IsArithmetic<T>::True || IsPointer<T>::True ||
                        IsFunction<T>::True || IsReference<T>::True > ::True;
  static const bool False = !True;
};
...
template <typename T> struct IsScalar {
  static const bool True = IsArithmetic<T>::True || IsEnum<T>::True ||
                           IsPointer<T>::True || IsMemberPointer<T>::True;
  static const bool False = !True;
};
...
template <typename T> struct IsPod {
  static const bool True =
      IsScalar<T>::True || IsVoid<T>::True || ClassTraits<T>::IsPod;
  static const bool False = !True;
};
template <typename T, size_t N> struct IsPod<T[N]> : IsPod<T> {};
...
template <typename T> struct HasTrivialConstructor {
  static const bool True =
      IsPod<T>::True || ClassTraits<T>::HasTrivialConstructor;
  static const bool False = !True;
};
...
template <typename T> struct HasTrivialDestructor {
  static const bool True =
      IsPod<T>::True || ClassTraits<T>::HasTrivialDestructor;
  static const bool False = !True;
};
...
template <typename T> struct HasTrivialAssign {
  static const bool True = IsPod<T>::True || ClassTraits<T>::HasTrivialAssign;
  static const bool False = !True;
};
...
template <typename T> struct HasTrivialCopy {
  static const bool True = IsPod<T>::True || ClassTraits<T>::HasTrivialCopy;
  static const bool False = !True;
};
...
// unqualified_type
namespace Private {
template <typename T> struct UnqualifiedPtr {
  typedef T ValueType;
};
template <typename T> struct UnqualifiedPtr<T *> {
  typedef T ValueType;
};
template <typename T> struct UnqualifiedPtr<const T *> {
  typedef T ValueType;
};
template <typename T> struct UnqualifiedPtr<volatile T *> {
  typedef T ValueType;
};
template <typename T> struct UnqualifiedPtr<const volatile T *> {
  typedef T ValueType;
};
}; // namespace Private
...
template <typename T> struct RemoveCV<volatile T> {
  typedef T ValueType;
};
template <typename T> struct RemoveCV<const volatile T> {
  typedef T ValueType;
};
template <typename T, size_t N> struct RemoveCV<T const[N]> {
  typedef T ValueType[N];
};
template <typename T, size_t N> struct RemoveCV<T volatile[N]> {
  typedef T ValueType[N];
};
template <typename T, size_t N> struct RemoveCV<T const volatile[N]> {
  typedef T ValueType[N];
};
...
template <typename T, size_t N> inline U32 ArraySize(T (&a)[N]) { return N; }

/// Computes the number of elements in a static array.
/// This is a type safe macro that will can be used in places where
/// the ArraySize template cannot be evaluated.
namespace Private {
template <typename T, size_t N> char (&_ArraySize(T (&)[N]))[N];
}
