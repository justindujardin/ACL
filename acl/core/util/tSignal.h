...
///
/// This allows handlers of a given event to say to the signal generator, "I
/// handled this message, and it is no longer appropriate for other listeners to
/// handle it"
...
public:
  ...
  bool isEmpty() { return mList.next == &mList; }
...
public:
  SignalBaseT() {}
  SignalBaseT(const SignalBaseT &base) : SignalBase() {
    mList.next = mList.prev = &mList;

    for (DelegateLink *ptr = base.mList.next; ptr != &base.mList;
         ptr = ptr->next) {
  ...
  template <class T, class U> void notify(T obj, U func, float order = 0.5f) {
    DelegateSig dlg(obj, func);
    notify(dlg, order);
  }

  template <class T> void notify(T func, float order = 0.5f) {
    DelegateSig dlg(func);
    notify(dlg, order);
  }

  template <class T, class U> void remove(T obj, U func) {
    DelegateSig compDelegate(obj, func);
    remove(compDelegate);
  }

  template <class T> void remove(T func) {
...
template <> class Signal<bool()> : public SignalBaseT<bool()> {
public:
  bool trigger() {
...
template <class A> class Signal<bool(A)> : public SignalBaseT<bool(A)> {
public:
  bool trigger(A a) {
...
template <> class Signal<void()> : public SignalBaseT<void()> {
public:
  void trigger() {
    SignalBase::DelegateLink *ptr = mList.next;
    while (ptr != &mList) {
...
template <class A> class Signal<void(A)> : public SignalBaseT<void(A)> {
public:
  void trigger(A a) {
    SignalBase::DelegateLink *ptr = this->mList.next;
    while (ptr != &this->mList) {
