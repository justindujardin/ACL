...
static S8 _DaysInMonth[13] = {0,  31, 28, 31, 30, 31, 30,
                              31, 31, 30, 31, 30, 31};
static S8 _DaysInMonthLeap[13] = {0,  31, 29, 31, 30, 31, 30,
                                  31, 31, 30, 31, 30, 31};
static S32 _DayNumber[13] = {0,   31,  59,  90,  120, 151, 181,
                             212, 243, 273, 304, 334, 365};
static S32 _DayNumberLeap[13] = {0,   31,  60,  91,  121, 152, 182,
...
Time Time::getCurrentTime() { return UnixTimeToTime((U32)time(NULL)); }

bool Time::set(S32 year, S32 month, S32 day, S32 hour, S32 minute, S32 second,
               S32 microsecond) {
  ...
  _time += S64((hour * OneHour) + (minute * OneMinute) + (second * OneSecond) +
               microsecond);
...
//-----------------------------------------------------------------------------
void Time::get(S32 *pyear, S32 *pmonth, S32 *pday, S32 *phour, S32 *pminute,
               S32 *psecond, S32 *pmicrosecond) const {
...
void Time::get(DateTime &dt) const {
  get(&dt.year, &dt.month, &dt.day, &dt.hour, &dt.minute, &dt.second,
      &dt.microsecond);
}
