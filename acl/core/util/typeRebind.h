...
template <typename CreatedType> class Behavior {
public:
  virtual ~Behavior() {}
...
template <typename BaseType> class AbstractCreator {
public:
...
public:
  /// Defaults to the RequiredBehavior for RebindConstraint<BaseType,
  /// DerivedType>, which is AlwaysCreateBehavior by default.
  Creator();
  ...
  template <typename BehaviorType> void withBehavior(BehaviorType *b);
...
template <typename BaseType> class Binding : public AbstractBinding {
public:
  ...
  template <typename DerivedType> Creator<BaseType, DerivedType> &to();
  ...
  /// readability purposes the preferred method is still
  /// to<Type>().withBehavior<Behavior>().
  ...
  /// Creates a DelegateCreateBehavior and immediately binds it to a new
  /// Creator. No further chaining is possible with this path.
...
public:
  ...
  template <typename BaseType> Binding<BaseType> &bind();
  ...
  /// necessary binding.
  template <typename BaseType> BaseType *create() const;

  /// Returns true if the type has been bound to something, false if the
  /// type has not been bound.
  template <typename BaseType> bool hasType() const;

  void swap(TypeRebind &other);

private:
  template <typename T> ACL_NO_INLINE U32 *getTypeId_() const {
    static U32 id;
    return &id;
  }

  template <typename BaseType> Binding<BaseType> *getBinding_() const;
...
template <typename BaseType, typename DerivedType> struct RebindConstraint {
...
template <typename CreatedType>
DelegateCreateBehavior<CreatedType>::DelegateCreateBehavior(
    const Delegate<CreatedType *()> &d)
    : mDelegate(d) {}

template <typename CreatedType>
void DelegateCreateBehavior<CreatedType>::assignDelegate(
    const Delegate<CreatedType *()> &d) {
  mDelegate = d;
}
...
template <typename CreatedType>
void SingletonBehavior<CreatedType>::assignDelegate(
    const Delegate<void(CreatedType *)> &d) {
  mDelegate = d;
}
...
Creator<BaseType, DerivedType>::Creator()
    : mBehavior(new
                typename RebindConstraint<BaseType,
                                          DerivedType>::RequiredBehavior()) {}

template <typename BaseType, typename DerivedType>
template <template <class> class BehaviorType>
Creator<BaseType, DerivedType>::Creator(BehaviorType<DerivedType> *b)
    : mBehavior(b) {
...
template <typename BaseType, typename DerivedType>
Creator<BaseType, DerivedType>::Creator(Behavior<DerivedType> *b)
    : mBehavior(b) {
...
template <typename BaseType, typename DerivedType>
template <typename BehaviorType>
void Creator<BaseType, DerivedType>::checkConstraint_(
    IntToType<true> hasConstraint) {
  AssertStatic(
      (IsSame<BehaviorType, typename RebindConstraint<BaseType, DerivedType>::
...
//--------------------------------------------------------------------------
template <typename BaseType> AbstractBinding *Binding<BaseType>::clone() const {
...
template <typename DerivedType, template <typename> class BehaviorType>
BehaviorType<DerivedType> &
Binding<BaseType>::to(BehaviorType<DerivedType> *b /* = NULL*/) {
...
template <typename BaseType> BaseType *Binding<BaseType>::create() const {
...
template <typename BaseType> bool TypeRebind::hasType() const {
  return mBindings.contains(getTypeId_<BaseType>());
}

template <typename BaseType> Binding<BaseType> &TypeRebind::bind() {
...
template <typename BaseType> BaseType *TypeRebind::create() const {
