...
S32 VirtualMountSystem::findByPattern( const Path &inBasePath, const String &inFilePattern, bool inRecursive, Vector<String> &outList, bool includeDirs )
{
   if (mUseParentFind)
      // use parent version
      return Parent::findByPattern(inBasePath, inFilePattern, inRecursive, outList, includeDirs);
   ...
   for (Vector<MountFS>::const_iterator itr = mMountList.begin(); itr != mMountList.end(); itr++)
   {
      if (itr->root.equal( inBasePath.getRoot(), String::NoCase ) )
      {
         ...
         Parent::findByPattern(inBasePath, inFilePattern, inRecursive, outList, includeDirs);
         _setFindByPatternOverrideFS(NULL);
      }
   }
   ...
   if (ret)
   {
      ...
      if (rootDict)
      {
         ...
         for (U32 i = 0; i < path.getDirectoryCount(); i++)
         {
            ...
            Vector<FileSystemRef>::iterator iter = Torque::Find(fsList.begin(), fsList.end(), fsRef);
