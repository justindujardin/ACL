...
public:
  ...
  /// @brief Closes and releases the resources associated with the dynamic
  /// library.
  /// @details After the destructor has executed, you should not call any
  /// functions returned by bind(), as their code may also have been unloaded.
  ~DLibrary();
  ...
  /// @return true if the library was successfully loaded, false if the library
  /// could not be loaded or file was not a valid path
  bool load(const String &file);

  /// @brief Returns a pointer to the given function
  /// @param name The function to lookup in the dynamic library.  May not be an
  /// empty string
  /// @returns The pointer to the given function, or NULL if the function could
  /// not be loaded or name was an empty string
