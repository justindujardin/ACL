...
public:
  ...
  /// was @c false.
  /// @returns Threading::Status_Deadlock if your implementation detected a
  /// deadlock condition. @note This return code is @e optional and the
  /// Platform::Mutex class provides its own deadlock detection when
  /// ACL_DEBUG_THREADING is defined.
  ...
  /// calling thread. @note This return code is @e optional and the
  /// Platform::Mutex class provides its own ownership check when
  /// ACL_DEBUG_THREADING is defined.
  /// @returns Threading::Status_PlatformError if the mutex could not be
  /// unlocked for any other reason.
