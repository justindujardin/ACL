...
void MacPlatformObject::initSystemInfo_(SystemInfo &systemInfo) {
  ...
  if (err == noErr) {
    ...
    systemInfo.fullOsName =
        String::ToString("Mac OS X %i.%i.%i", systemInfo.majorVersion,
                         systemInfo.minorVersion, systemInfo.bugfixVersion);
  } else {
    systemInfo.majorVersion = systemInfo.minorVersion =
        systemInfo.bugfixVersion = -1;
  ...
  sysctlbyname("hw.ncpu", &systemInfo.Processor.numLogicalProcessors, &theSize,
               NULL, 0);
