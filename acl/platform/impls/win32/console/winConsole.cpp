...
#include "platform/impls/win32/console/winConsole.h"

// Create static singleton so we always get the consumer signal
static WinConsole * sgAlwaysCreateOne = WinConsole::get();
...
WinConsole * WinConsole::get()
{
   static WinConsole winconsole;
   return &winconsole;
}
...
void WinConsole::processConsoleLine(const char *consoleLine)
{
   ...
   {
      inbuf[inpos] = 0;
      // Use global printf for automated test case.
      ::printf("%s\n", consoleLine);
   }
}
