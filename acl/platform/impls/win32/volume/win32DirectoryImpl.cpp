...
namespace Internal_ {
namespace Win32 {
extern void _CopyStatAttributes(const WIN32_FIND_DATAW &info,
...
void Win32DirectoryImpl::updateStatus() {
  ...
  case ERROR_SHARING_VIOLATION:
    mStatus = FileNode::SharingViolation;
    break;
...
bool Win32DirectoryImpl::open(const Path &path) { return true; }
...
bool Win32DirectoryImpl::read(const Path &path,
                              FileNode::Attributes &attributes) {
  WIN32_FIND_DATA info;
  if (!mHandle) {
    mHandle =
        ::FindFirstFileW((LPCWSTR)(path.getFullPath() + "\\*").utf16(), &info);

    if (mHandle == NULL)
      return false;
  } else if (!::FindNextFileW(mHandle, &info))
    return false;
