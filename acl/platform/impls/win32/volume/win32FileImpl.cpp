...
void Win32FileImpl::updateStatus() {
  ...
  case ERROR_HANDLE_DISK_FULL:
    mStatus = FileNode::FileSystemFull;
    break;
...
bool Win32FileImpl::open(const Path &path, File::AccessMode mode) {
  ...
  Mode &m = (mode == File::Read)        ? Modes[0]
            : (mode == File::Write)     ? Modes[1]
            : (mode == File::ReadWrite) ? Modes[2]
                                        : Modes[3];

  mHandle = ::CreateFileW(
      (LPCWSTR)path.getFullPath().utf16(), m.mode, m.share, NULL, m.open,
