...
bool Win32FileSystemChangeNotifierImpl::addNotification(const Path &dir) {
  HANDLE changeHandle = ::FindFirstChangeNotificationW(
      ...
      FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_LAST_WRITE |
          FILE_NOTIFY_CHANGE_ATTRIBUTES); // watch file write changes

  if (changeHandle == INVALID_HANDLE_VALUE || changeHandle == NULL) {
    // TODO:Console
    // Con::errorf("[Win32FileSystemChangeNotifier::internalAddNotification] :
    // failed on [%s] [%d]", dir.getFullPath().c_str(), GetLastError());
    return false;
  }
...
void Win32FileSystemChangeNotifierImpl::process() {
  ...
  DWORD dwWaitStatus =
      WaitForMultipleObjects(numHandles, mHandles.address(), FALSE, 0);
  ...
  if (dwWaitStatus <= (WAIT_OBJECT_0 + numHandles - 1)) {
    ...
    // NOTE: we do this before letting the volume system check mod times so we
    // don't miss any.
    //    It is going to loop over the files and check their mod time vs. the
    //    saved time. This may result in extra calls to
    //    internalNotifyDirChanged(), but it will simpley check mod times again.
