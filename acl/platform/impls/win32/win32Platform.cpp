...
#ifndef UNICODE
#error                                                                         \
    "ERROR: You must have UNICODE defined in your preprocessor settings (ie, /DUNICODE)!"
#endif

namespace ACLib {
namespace Platform {
namespace Internal_ {
void InitCreator() { RegisterPlatform<Win32PlatformObject>(); }
...
Win32PlatformObject::Win32PlatformObject() {
  ...
  factory.bind<FileSystemChangeNotifierImpl>()
      .to<Win32FileSystemChangeNotifierImpl>();
}
...
void Win32PlatformObject::abort(U32 code) { ExitProcess(code); }

void Win32PlatformObject::debugBreak() { DebugBreak(); }
...
void Win32PlatformObject::sleep(U32 ms) { Sleep(ms); }

Path Win32PlatformObject::getExecutablePath() {
  static String cen;
  if (cen.isEmpty()) {
    WCHAR cen_buf[2048];
    GetModuleFileNameW(NULL, cen_buf, 2047);
    cen = PathToPlatform((UTF16 *)cen_buf);
...
String Win32PlatformObject::getUserDataDirectory() {
  ...
  return PathToPlatform((UTF16 *)szBuffer);
}

String Win32PlatformObject::getUserHomeDirectory() {
  WCHAR szBuffer[MAX_PATH];
  if (SHGetFolderPathW(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT,
                       szBuffer) != S_OK)
    if (SHGetFolderPathW(NULL, CSIDL_COMMON_DOCUMENTS, NULL, SHGFP_TYPE_CURRENT,
                         szBuffer) != S_OK)
      return "";
...
void Win32PlatformObject::restartInstance() {
  ...
  if (copiedSize == BufSize - 1) {
    // Con::errorf("Win32PlatformObject::restartInstance - apparently we need
    // more than %d TCHAR's for the file name", BufSize);
    return;
  }
...
void Win32PlatformObject::postQuitMessage(U32 code) { PostQuitMessage(code); }
...
bool Win32PlatformObject::touchFile(const Path &path) {
