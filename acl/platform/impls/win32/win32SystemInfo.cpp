...
#endif
  // From http://www.intel.com/assets/pdf/appnote/241618.pdf
  if (properties1 & BIT(20) || properties1 & BIT(19)) return 4;
...
void Win32PlatformObject::initSystemInfo_(SystemInfo &info) {
  ...
  if (viSuccess) {
    ...
    info.fullOsName = String::ToString("Windows %u.%u.%u", info.majorVersion,
                                       info.minorVersion, info.bugfixVersion);
  } else {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,
    ...
    if (RegQueryValueExA(regKey, "~Mhz", 0, &regType, (LPBYTE)&regMhz,
