...
struct PlatformObject::Internal {
  ScopedPtr<SystemInfo> systemInfo;
  Vector<TypeRebind *> factoryStack;
...
PlatformObject *GetPlatform() {
  ...
  if (!inited) {
    ...
    PlatformObject *p =
        Internal_::GetPlatformFactory().create<PlatformObject>();
...
PlatformObject::PlatformObject() : mImpl(new Internal) {
  Assert::Get().pushImpl(new Internal_::PlatformAssertImpl);
  mImpl->factoryStack.push_back(new TypeRebind);
}

PlatformObject::~PlatformObject() {
  using namespace ACLib;
  ForEach(mImpl->factoryStack.begin(), mImpl->factoryStack.end(),
          &DeleteSingle::destroy<TypeRebind>);
}
...
void PlatformObject::popFactory() {
  AssertFatal(mImpl->factoryStack.size() > 1,
              "Can't pop last factory in the stack");
  if (mImpl->factoryStack.size() == 1)
    return;

  TypeRebind *r = mImpl->factoryStack.last();
  mImpl->factoryStack.pop_back();
  delete r;
}

const TypeRebind &PlatformObject::getFactory() const {
  return *mImpl->factoryStack.last();
}

TypeRebind &PlatformObject::getProtectedFactory() {
  return *mImpl->factoryStack.last();
}

MathStateKnown::MathStateKnown()
    : mOldState(GetPlatform()->getMathControlState()) {
