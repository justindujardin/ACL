...
/// Except where otherwise noted, all classes in Platform are threadsafe such that
...
/// every class in the Platform namespace maintains a pointer to an "Implementation"
/// class in the Platform::Internal_ namespace.  It is this implementation class
/// which is subclassed and instantiated to provide platform specific functionality.
/// In addition to the Platform::Internal_ implementation class, almost all Platform
...
/// the Platform::Internal_ implementation object.
///
/// This design gives the user great flexibility in using Platform, as the classes
/// exposed to the user in the Platform namespace are all concrete classes and
/// can therefore be instantiated with new or created directly on the stack.
///
/// This design also gives the implementor of Platform (or a Platform implementation)
/// a great deal of flexibility, as internal details, or even the internal API itself,
/// can be changed without requiring changes to code outside of Platform.
///
...
/// object) so Platform may not be suitable for use on certain embedded systems.
/// This design also means that almost all Platform objects require at least two
/// additional allocations from the heap (currently Platform uses the global
/// operator new) which could also be unnacceptable on certain systems.  In the future
/// Platform may provide a way for the application to specify an allocator for use
