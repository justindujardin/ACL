...
public:
  ...
  /// @remarks If ACL_DEBUG_THREADING is defined this method will assert if it
  /// is called more than once from the same thread (with block = true).
  ...
  /// @returns Threading::Status_Busy if block was false and the mutex was
  /// already locked by another thread.
  /// @returns Threading::Status_Deadlock if a deadlock condition was detected
  /// (detection enabled if ACL_DEBUG_THREADING is defined)
  /// @returns Threading::Status_ObjectInvalid if the mutex was not successfully
  /// created. This error is probably non-recoverable.
  /// @returns Threading::Status_PlatformError under implementation defined
  ...
  /// @remarks If ACL_DEBUG_THREADING is defined this method will assert if it
  /// is called from a thread which did not lock the mutex.
  /// @returns Threading::Status_NoError if the mutex was unlocked
  /// @returns Threading::Status_Permission if the calling thread does not own
  /// the mutex. (detection enabled if ACL_DEBUG_THREADING is defined)
  /// @returns Threading::Status_ObjectInvalid if the mutex was not successfully
  /// created. This error is probably non-recoverable.
  /// @returns Threading::Status_PlatformError under implementation defined
  /// circumstances. This error is probably non-recoverable.
  ...
  public:
    ScopedLock(Mutex &m) : mMutex(m) { mMutex.lock(); }
