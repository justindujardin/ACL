...
struct Thread::Internal {
  ...
  Internal(const Thread::StartDelegate &d)
      : impl(GetPlatform()->getFactory().create<Internal_::ThreadImpl>()),
        delegate(d), messageQueue(), running(false), hasStarted(false),
        isRunningBlocker() {}
...
Threading::Status Thread::start() {
  ...
  Internal_::ThreadImpl::Param *p = new Internal_::ThreadImpl::Param(
      mImpl->delegate, mImpl->messageQueue, mImpl->impl.get(), this);
...
bool Thread::isRunning() {
  if (mImpl->running) {
    ...
    mImpl->running =
        mImpl->impl->gateway.acquire(false) == Threading::Status_Busy;
    if (!mImpl->running)
      mImpl->impl->gateway.release();
  }
...
S32 Thread::getReturnCode() const { return mImpl->impl->getReturnCode(); }

Thread::MessageQueue &Thread::getMessageQueue() { return mImpl->messageQueue; }
} // namespace Platform
} // namespace ACLib