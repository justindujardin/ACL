...
/// @brief Classes used when creating and working with a multithreaded
/// application.
/// @details Every class in this module is threadsafe such that two threads can
/// concurrently invoke methods on the same object, in addition to the
/// guarantees already made by Platform.

namespace Platform {
/// @ingroup p2thread
/// @brief A wrapper around platform native threads.
/// @details Unless otherwise noted, it is safe for two threads to concurrently
/// invoke methods on the same Thread object.
...
public:
  ...
  public:
    ...
    /// @param block If true block the calling thread until a message is
    /// available
  ...
  /// @brief This is the required function signature for functions started on
  /// another thread.
  ...
  /// @brief Attempts to start a new thread which will call the delegate given
  /// to the constructor.
  ...
  /// started.
  /// @returns Threading::Status_Again if start() has already succeeded on this
  /// object.
  ...
  /// @brief Returns true if the thread is currently running, otherwise returns
  /// false.
  /// @details Sleeping counts as running. This call will directly poll the
  /// state of the thread, so its return may change even if finish() is not
  /// called.
  bool isRunning();
  ...
  /// @details If called when isRunning() will return true the return value is
  /// undefined.
  ...
  /// posted before the thread has started will be in the queue when the thread
  /// starts.
