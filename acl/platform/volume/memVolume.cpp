...
namespace ACLib
{
   ...
   namespace Mem
   {
      ...
      MemDirectory* MemFileSystem::getParentDir(const Path& path, FileNodeRef& parentRef)
      {
         ...
         {
            // empty file name, so input path maps to a path, not a filename.  we want the parent of the 
            // path.  
            // the following looks dubious, and it is.  by assigning the path string to the manglePath,
            // the last element will be treated as a filename, so then manglePath.getPath() is the actual
            // parent path directory.  hax.
