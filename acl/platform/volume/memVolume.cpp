...
MemFileSystem::~MemFileSystem() { delete mRootDir; }
...
//
MemDirectory *MemFileSystem::getParentDir(const Path &path,
                                          FileNodeRef &parentRef) {
  String parentPath;
  if (path.getFullFileName().isEmpty()) {
    // empty file name, so input path maps to a path, not a filename.  we want
    // the parent of the path. the following looks dubious, and it is.  by
    // assigning the path string to the manglePath, the last element will be
    // treated as a filename, so then manglePath.getPath() is the actual parent
    // path directory.  hax.
    Path manglePath = path.getFullPath();
    parentPath = manglePath.getRoot() + ":" + manglePath.getPath();
  } else {
    parentPath = path.getRoot() + ":" + path.getPath();
  }

  parentRef = mRootDir->resolve(parentPath);
  if (parentRef.isNull())
    return NULL;
...
Path MemFile::getName() const { return mFileData->mPath; }

FileNode::Status MemFile::getStatus() const { return mStatus; }
...
Path MemDirectory::getName() const { return mDirectoryData->mPath; }
...
bool MemDirectory::close() { return true; }
...
bool MemDirectory::read(Attributes *entry) {
  ...
  AssertFatal(mSearchIndex > mDirectoryData->mDirectories.size(),
...
FileNode::Status MemDirectory::getStatus() const { return mStatus; }
} // Namespace Mem
