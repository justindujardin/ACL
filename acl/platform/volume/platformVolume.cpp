...
namespace ACLib {
namespace Platform {
// utility class for passing a PlatformFile object to functions that require a
// Stream interface
class PlatformFileStream : public Stream {
public:
  PlatformFile *mFile;
  PlatformFileStream(PlatformFile *file) { mFile = file; };
  ...
  virtual bool hasCapability(const Capability) const { return true; }
  /// Gets the position in the stream
  virtual U32 getPosition() const { return mFile->getPosition(); }
  /// Sets the position of the stream.  Returns if the new position is valid or
  /// not
  virtual bool setPosition(const U32 in_newPosition) {
    return mFile->setPosition(in_newPosition, FS::File::Begin);
  }
  /// Gets the size of the stream
  virtual U32 getStreamSize() { return mFile->getSize(); }
};
...
struct PlatformDirectory::Config {
  ...
  Config(const Path &path_, const Path &fullPath_,
         const StrongRefPtr<Internal_::FileSystemImpl> &fileSystemImpl_)
...
FileNodeRef PlatformFileSystem::resolve(const Path &path) {
  ...
  if (attributes.flags & FileNode::Directory)
    return new PlatformDirectory(
        PlatformDirectory::Config(path, file, mImpl->impl));
...
FileNodeRef PlatformFileSystem::create(const Path &path, FileNode::Mode mode) {
  if (mode & FileNode::File)
    return new PlatformFile(PlatformFile::Config(
        path, buildFileName(mImpl->volume, path), mImpl->impl));
...
struct PlatformDirectory::Internal {
  ScopedPtr<Internal_::DirectoryImpl> impl;
  ...
  Internal(const PlatformDirectory::Config &config)
      : impl(GetPlatform()->getFactory().create<Internal_::DirectoryImpl>()),
        fileSystemImpl(config.fileSystemImpl), path(config.path),
...
PlatformDirectory::~PlatformDirectory() { mImpl->impl->close(); }

Path PlatformDirectory::getName() const { return mImpl->path; }
...
bool PlatformDirectory::getAttributes(Attributes *attributes) {
  if (!mImpl->fileSystemImpl->getAttributesForPath(*attributes,
                                                   mImpl->fullPath)) {
...
U32 PlatformDirectory::calculateChecksum() { return 0; }

struct PlatformFile::Internal {
  ScopedPtr<Internal_::FileImpl> impl;
  StrongRefPtr<Internal_::FileSystemImpl> fileSystemImpl;
  Path path;
  Path fullPath;
...
PlatformFile::~PlatformFile() { mImpl->impl->close(); }

Path PlatformFile::getName() const { return mImpl->path; }
...
U32 PlatformFile::getPosition() {
  if (mImpl->impl->mStatus == FileNode::Open ||
      mImpl->impl->mStatus == FileNode::EndOfFile)
...
struct PlatformFileSystemChangeNotifier::Internal {
  ScopedPtr<Internal_::FileSystemChangeNotifierImpl> impl;
  Vector<Path> paths;

  Internal()
      : impl(GetPlatform()
                 ->getFactory()
                 .create<Internal_::FileSystemChangeNotifierImpl>()) {}
};

PlatformFileSystemChangeNotifier::PlatformFileSystemChangeNotifier(
    FileSystem *fs)
    : FileSystemChangeNotifier(fs), mImpl(new Internal) {
  mImpl->impl->onDirChanged.bind(
      this, &PlatformFileSystemChangeNotifier::onDirChanged);
}
...
bool PlatformFileSystemChangeNotifier::internalAddNotification(
    const Path &dir) {
...
bool PlatformFileSystemChangeNotifier::internalRemoveNotification(
    const Path &dir) {
  for (S32 i = 0; i < mImpl->paths.size(); i++) {
    if (mImpl->paths[i] == dir) {
      String fullPath = PathToOS(mFS->mapTo(dir));
