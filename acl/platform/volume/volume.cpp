...
namespace ACLib
{
   ...
   void  FileSystemChangeNotifier::startNotifier()
   {
      ...
      for ( ; itr != mDirMap.end(); ++itr )
      {
         ...
         for ( U32 i = 0; i < fileList.getSize(); i++ )
         {
            ...
            AssertWarn( success, "[FileSystemChangeNotifier::startNotifier] failed to get attributes on file" );
   ...
   {
      // TODO: Logging
      //Con::warnf("[FileSystemChangeNotifier::internalNotifyDirChanged] : [%s]", dir.getFullPath().c_str());
      ...
      for ( U32 i = 0; i < fileList.getSize(); i++ )
      {
         ...
         AssertFatal( success, "[FileSystemChangeNotifier::internalNotifyDirChanged] failed to get attributes on file" );
         ...
         if ( attr.mtime > fInfo.savedLastModTime )
         {
            ...
            // Queue up notifications.  If a file is removed from the monitored list, inside of a signal,
   ...
   S32 MountSystem::findByPattern( const Path &inBasePath, const String &inFilePattern, bool inRecursive, Vector<String> &outList, bool includeDirs )
   {
   ...
   namespace FS
   {
      ...
      {
         return sgMountSystem.findByPattern(inBasePath, inFilePattern, inRecursive, outList);
      }
