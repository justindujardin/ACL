...
typedef struct PluginDescription {
  S32 platSize;             ///< must be set manually, e.g. platDesc.platSize =
                            ///< sizeof(PluginPlatformServices);
...
typedef struct PluginExports {
  ...
  /// Register a Plugin
  /// @param platServ PluginPlatformServices pointer to the
  ...
  /// @return true if unload is successful. if canCancel is true, returning
  /// false
  ...
  /// @return A PluginDescription structure containing the information necessary
  /// to
