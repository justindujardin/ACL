...
#ifndef TORQUE_CORE_UTIL_TYPEREBIND_H_
#define TORQUE_CORE_UTIL_TYPEREBIND_H_
...
#include "../containers/tDictionary.h"
...
namespace Torque
{
   ...
   {
      /// Class contraints
      AssertStaticNamespace(Torque::IsAbstract<CreatedType>::False, 
   ...
   {
      /// Class contraints
      AssertStaticNamespace((Torque::IsConvertible<DerivedType*, BaseType*>::True),
   ...
   private:
      template<typename BehaviorType>
      void checkConstraint_(Torque::IntToType<true> hasConstraint);
      
      template<typename BehaviorType>
      void checkConstraint_(Torque::IntToType<false> noConstraint);
      
      // Does not enforce constraints
      Creator(Behavior<DerivedType>* b);
      
      Torque::ScopedPtr<Behavior<DerivedType> > mBehavior;
   };
   ...
   private:
      Torque::ScopedPtr<AbstractCreator<BaseType> > mCreator;
   };
   ...
   private:
      template<typename T>
      TORQUE_NO_INLINE U32* getTypeId_() const
      {
         static U32 id;
         return &id;
      }
   ...
   Creator<BaseType, DerivedType>::Creator() : 
      mBehavior(new typename Torque::RebindConstraint<BaseType, DerivedType>::RequiredBehavior())
   {
   }
   ...
   Creator<BaseType, DerivedType>::Creator(BehaviorType<DerivedType>* b) : mBehavior(b)
   {
      checkConstraint_<BehaviorType<DerivedType> >
         (Torque::IntToType<Torque::RebindConstraint<BaseType, DerivedType>::ForceBehavior>());
   }
   ...
   void Creator<BaseType, DerivedType>::checkConstraint_(Torque::IntToType<true> hasConstraint)
   {
      AssertStatic((Torque::IsSame<BehaviorType, 
         typename Torque::RebindConstraint<BaseType, DerivedType>::RequiredBehavior>::True),
         rebind_constraint_failed_incorrect_behavior);
   }
      
   template<typename BaseType, typename DerivedType> template<typename BehaviorType>
   void Creator<BaseType, DerivedType>::checkConstraint_(Torque::IntToType<false> noConstraint)
   {
   }
   ...
   Creator<BaseType, DerivedType>& Binding<BaseType>::to()
   {
      ...
      AssertStatic((Torque::IsConvertible<DerivedType*, BaseType*>::True),
