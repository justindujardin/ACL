...
TEST(PathTest, BasicStuff) {
  ...
  // String must end with / otherwise Path will treat Garbage as a filename and
  // the append path will have no effect
...
TEST(PathTest, CompressPath) {
  ...
  EXPECT_TRUE(Path::CompressPath("foo\\bar//..").equal("foo"));
  EXPECT_TRUE(
      Path::CompressPath("foo/./././///\\//./bar/.././///.//\\///./\\/.")
          .equal("foo"));
  EXPECT_TRUE(Path::CompressPath("foo/\\//\\///\\././.\\.\\/\\//./bar////././"
                                 ".\\.././///.//\\///./\\/.")
                  .equal("foo"));
}
