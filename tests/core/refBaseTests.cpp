...
// Test WeakRefPtr and StrongRefPtr (WeakRefUnion and StrongRefUnion still to
// come)
...
TEST(StrongRefTest, AssignAndDestroy) {
  // Other possible tests: make sure WeakRefBase::WeakReference is cleared when
  // it should be
...
TEST(StrongRefTest, OnePtrTwoRefs) {
  ...
  // result should be we now point at strongRef2 and strongRef was deleted
  // (note: only check that a deletion occurred)
  //"strong pointer not set properly"
