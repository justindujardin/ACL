...
CreateUnitTest(Check_advanceTime, "Platform/Time/advanceTime")
{
   void run()
   {
      U32 time = SimWorld::getRootWorld()->getCurrentTime();
...
// 1/08/09 - Disabled this test because it runs the main process loop while we are in the main process loop
// when executed from the console.  There is no easy way to detect that we are currently in the main process loop
// so we can't skip this test in situations where it could cause issues. -AlexS
