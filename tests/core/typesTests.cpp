...
#define CheckType(typeName, expectedSize) \
            AssertStatic( sizeof(typeName) == expectedSize, Wrong_size_for_a_##typeName##_expected_##expectedSize);
...
#define CheckEndianRoundTrip(type, b_or_l) \
   EXPECT_TRUE( type##Test == convert##b_or_l##EndianToHost(convertHostTo##b_or_l##Endian(type##Test)));
