...
public:
  ...
  TestFileSystemImpl() {
    getAttributesForPathCt = removeFileCt = removeDirectoryCt =
        createDirectoryCt = 0;
  }

  bool getAttributesForPath(FS::FileNode::Attributes &attributes,
                            const Path &path) {
  ...
  bool move(const Path &from, const Path &to) { return false; }
};
} // namespace TestPlatform
} // namespace ACLib
#endif
