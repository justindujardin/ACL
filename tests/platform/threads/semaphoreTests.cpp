...
TEST(SemaphoreBehavior,ConstructorMaxCount)
{
   ...
   EXPECT_TRUE(assertImpl->checkAndReset(0));//, "Expected that a semaphore created with max count > 0 would not assert");

   // Specify that maxcount goes directly to the impl
   EXPECT_TRUE(testImpl->maxCount == 1);//, "Expected that maxcount > 0 would go directly to impl");
   ...
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that a semaphore created with max count == 0 would assert");

   // Specify that we recover from the assert by setting maxCount to S32_MAX
   EXPECT_TRUE(testImpl->maxCount == S32_MAX);//, "Expected that maxcount == 0 would be set to S32_MAX");
   ...
   EXPECT_TRUE(assertImpl->checkAndReset(2));//, "Expected that a semaphore created with max count < 0 would assert");
...
TEST(SemaphoreBehavior,ConstructorInitialCountVsMaxCount)
{
   ...
   TestSemaphoreImpl* testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(0));//, "Expected that a semaphore created with initialCount < maxCount would not assert");

   // Specify that initialcount goes directly to the impl
   EXPECT_TRUE(testImpl->initialCount == 1);//, "Expected that initial count < maxCount would go directly to impl");
   ...
   s = new Semaphore(2, 2);
   testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(0));//, "Expected that a semphoare with initialCount == maxCount would be valid");

   // Specify that initialCount goes directly to impl
   EXPECT_TRUE(testImpl->initialCount == 2);//, "Expected that initial count == maxCount would go directly to impl");
   ...
   s = new Semaphore(3, 2);
   testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that a semaphore created with initialCount > maxCount would assert");

   // Specify that we can recover from an assertion by providing initialCount = maxCount
   EXPECT_TRUE(testImpl->initialCount == testImpl->maxCount);//, "Expected that initialCount == maxCount when given as > maxCount");
...
TEST(SemaphoreBehavior,ConstructorInitialCountVs0)
{
   ...
   AutoPtr<Semaphore> s(new Semaphore(1, 2));
   TestSemaphoreImpl* testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(0));//, "Expected that a semaphore created with initialCount > 0 would not assert");
   EXPECT_TRUE(testImpl->initialCount == 1);//, "Expected that initial count > 0 would go directly to impl");
   ...
   s = new Semaphore(0, 2);
   testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(0));//, "Expected that a semphoare with initialCount == 0 would be valid");

   // Specify that initialCount goes directly to impl
   EXPECT_TRUE(testImpl->initialCount == 0);//, "Expected that initial count == 0 would go directly to impl");
   ...
   s = new Semaphore(-1, 2);
   testImpl = fixture->semaphoreImpl;
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that a semphoare with initialCount < 0 would assert");

   // Specify that we recover by setting initialCount = maxCount
   EXPECT_TRUE(testImpl->initialCount == testImpl->maxCount);//, "Expected that initial count < 0 would be set to maxCount");
};
...
TEST(SemaphoreBehavior,Valid)
{
   ...
   EXPECT_TRUE(testImpl->initCt == 1);//, "Expected that Semaphore::Semaphore() would call SemaphoreImpl::init");
   ...
   EXPECT_TRUE(s->release() == Threading::Status_PlatformError);//, "Expected that Semaphore::release would parrot the impl");
   EXPECT_TRUE(testImpl->releaseCt == 1);//, "Expected that Semaphore::release would call SemaphoreImpl::release");

   testImpl->releaseSuccess = Threading::Status_NoError;
   EXPECT_TRUE(s->release() == Threading::Status_NoError);//, "Expected that Semaphore::release would parrot the impl");
   EXPECT_TRUE(testImpl->releaseCt == 2);//, "Expected that Semaphore::release would call SemaphoreImpl::release");

   // Specify that acquire should parrot the impl
   testImpl->acquireSuccess = Threading::Status_PlatformError;
   EXPECT_TRUE(s->acquire() == Threading::Status_PlatformError);//, "Expected that Semaphore::acquire would parrot the impl");
   EXPECT_TRUE(testImpl->acquireCt == 1);//, "Expected that Semaphore::acquire would call SemaphoreImpl::acquire");

   testImpl->acquireSuccess = Threading::Status_NoError;
   EXPECT_TRUE(s->acquire() == Threading::Status_NoError);//, "Expected that Semaphore::acquire would parrot the impl");
   EXPECT_TRUE(testImpl->acquireCt == 2);//, "Expected that Semaphore::acquire would call SemaphoreImpl::acquire");
};
...
TEST(SemaphoreBehavior,Invalid)
{
   ...
   Semaphore fail(1);
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that Semaphore constructor would assert if SemaphoreImpl failed to init properly");

   TestSemaphoreImpl* testImpl = fixture->semaphoreImpl;
   testImpl->acquireSuccess = Threading::Status_NoError;
   // Specify that a blocking acquire fails and asserts if SemaphoreImpl is invalid
   EXPECT_TRUE(fail.acquire() == Threading::Status_ObjectInvalid);//, "Expected that Semaphore::acquire would fail if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that Semaphore::acquire would assert if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(testImpl->acquireCt == 0);//, "Expected that SemaphoreImpl::acquire would not be called if SemaphoreImpl failed to init properly");

   // Specify that a non-blocking acquire fails and asserts if SemaphoreImpl is invalid
   EXPECT_TRUE(fail.acquire(false) == Threading::Status_ObjectInvalid);//, "Expected that non-blocking Semaphore::acquire would fail if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that non-blocking Semaphore::acquire would assert if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(testImpl->acquireCt == 0);//, "Expected that SemaphoreImpl::acquire would not be called if SemaphoreImpl failed to init properly");

   // Specify that release asserts if SemaphoreImpl is invalid
   EXPECT_TRUE(fail.release() == Threading::Status_ObjectInvalid);//, "Expected that Semaphore::release would fail if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(assertImpl->checkAndReset(1));//, "Expected that Semaphore::release would assert if SemaphoreImpl failed to init properly");
   EXPECT_TRUE(testImpl->releaseCt == 0);//, "Expected that SemaphoreImpl::release would not be called if SemaphoreImpl failed to init properly");
};
