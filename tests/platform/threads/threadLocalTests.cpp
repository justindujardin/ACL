...
/// Verifies that a thread local stores different values for each accessing
/// thread.
...
S32 work(Thread::MessageQueue &messageQueue) {
  ...
  EXPECT_TRUE(reinterpret_cast<U32>(l.get()) ==
              7); //, "Thread local value incorrect on thread");
  return 0;
}
...
TEST(ThreadLocal, GetSet) {
  ...
  t.finish();
  EXPECT_TRUE(reinterpret_cast<U32>(l.get()) ==
              42); //, "Thread local value incorrect");
}
}; // namespace ThreadLocalData
