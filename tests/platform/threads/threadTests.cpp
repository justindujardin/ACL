...
/// Basic validation of thread functionality.  Verifies that impl properly
/// reports running state that impl calls the given method, and that impl passes
/// in userData and dataSize, and that imple reports the return value of the
/// thread.
...
TEST(Thread, BasicUsage) {
  ...
  t.start();
  t.finish();
  GetPlatform()->sleep(10); // Give the OS a moment to start/end the tread.  We
                            // shouldn't assume this is sync.
  EXPECT_FALSE(t.isRunning()); //"Thread in a running state after finish()");
  EXPECT_TRUE(didWork);        //"Thread not executed");
  EXPECT_TRUE(t.getReturnCode() ==
              _magicNumber); //, "Thread did not return magic number");
}
} // namespace BasicUsage

/// Verifies that when a thread is deleted it posts a Terminate message to the
/// message queue and waits until that message is received and acted upon.
namespace DeleteTermination {
S32 work(Thread::MessageQueue &messageQueue) {
  // Sleep in a tight loop.
  // Certain implementations can only cancel at entry to certain API calls, such
  // as sleep.
...
/// Verifies that Thread can post user defined messages and that threads can
...
/// Verify that the implementation polls the thread when isRunning is called,
/// and that given a thread that will finish in a finite amount of time
/// isRunning begins to return false in a finite amount of time.
...
S32 dummy(Thread::MessageQueue &) { return 0; }
...
TEST(ThreadBehavior, StartParrotsImplStart) {
  ...
  //, "Expected that Thread::start() would succeed if ThreadImpl::start()
  // succeeds");
...
TEST(ThreadBehavior, IsRunningParrotsImplStart) {
  ...
  fixture->threadImpl->startSuccess = Threading::Status_PlatformError;
  t.start();
  //, "Expected that Thread::isRunning() would be false if ThreadImpl::start
  // fails"
  ...
  //, "Expected that Thread::isRunning() would be true if ThreadImpl::start
  // succeeds"
...
S32 someFunc(Thread::MessageQueue &) { return 0; }
...
TEST(ThreadBehavior, StartDelegate) {
  ...
  // "Expected that delegate passed to ThreadImpl::start would be same as
  // delegate passed to Thread::Thread()"
...
TEST(ThreadBehavior, InvalidStartDelegate) {
  ...
  fixture->threadImpl->startSuccess = Threading::Status_NoError;
  //, "Expected that thread with empty startdelegate would return
  // DelegateInvalid on start()"
  EXPECT_TRUE(t.start() == Threading::Status_DelegateInvalid);
  //, "Expected that thread with invalid startdelegate would not call
  // ThreadImpl::start()"
  EXPECT_TRUE(fixture->threadImpl->startCt == 0);
}
...
S32 someFunc(Thread::MessageQueue &) { return MAGIC_NUMBER; }
...
TEST(ThreadBehavior, ReturnCode) {
  ...
  //, "Expected Thread::getReturnCode() would return the same value returned by
  // the delegate"
